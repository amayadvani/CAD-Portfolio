// 3D CAD Viewer using Three.js - V6
// Fixed: Uses STLLoader with arrayBuffer for proper binary STL support
...
class CADViewer {
 ...
 this.setupResize();
 this.animate();
 console.log('CAD Viewer V6 initialized (STLLoader)');
 }
 ...
 // Load model using STLLoader with ArrayBuffer (handles binary and ASCII STL)
 ...
 console.log('STL binary loaded, size:', buffer.byteLength, 'bytes');
 const geometry = THREE.STLLoader ? THREE.STLLoader.prototype.parse(buffer) : null;
 if (!geometry) {
 throw new Error('STLLoader failed to parse buffer');
 }
